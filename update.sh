...
for platform in ${!updates[@]}; do
  ...
  hash=$(nix hash convert --hash-algo sha256 "$source")
  update-source-version cursor $version $hash "$url" --system=$platform --ignore-same-version --source-key="sources.$platform" --file=package.nix
done
